...
<!--          </el-popover>-->
      ...
      <el-table v-loading="is加载中" :data="Data.list" border style="width: 100% ;white-space: pre-wrap;"
        ...
        <el-table-column prop="name" label="用户名" width="210" show-overflow-tooltip=""/>
        ...
        <el-table-column prop="createdAt" label="时间" width="160">
          <template #default="scope">
            {{ 时间_时间戳到时间(scope.row.createdAt) }}
          </template>
        </el-table-column>
        <el-table-column prop="number" label="变化值" width="160">
          <template #default="scope">
            <el-tag
                 :type="scope.row.number<=0?'success':'primary'">
              {{scope.row.number}}
            </el-tag>
          </template>
        </el-table-column>
...
import {签到日志api} from "@/api/签到日志api.js";
import {
  时间_时间戳到时间,
  时间_取现行时间戳,
  时间_计算天时分秒提示,
...
import {Del批量删除App, GetAppIdNameList} from "@/api/应用列表api";
...
const on批量删除 = async (type: number) => {
  ...
  console.log(ids)
  const res = await 签到日志api.delete({"ids": ids})
  console.log(res)
  if (res.code == 10000) {
...
const onGetLogVipNumberList = async () => {
  is加载中.value = true
  //let aa =  对象_搜索条件.value //这样不行,实际是引用,需要复制一份变了
  ...
  const res = await 签到日志api.getList(aa)
...
const on表格列宽被改变 = (newWidth: any, oldWidth: any, columns: any, event: any) => {
  ...
  localStorage.setItem('列宽_签到日志', JSON.stringify(局_列宽数组));
}
const on表格列宽初始化 = () => {

  let 局_列宽数组文本 = localStorage.getItem('列宽_签到日志')
  if (局_列宽数组文本 != null) {
...
onMounted(async () => {
  ...
  onReset()
  if (Store.state.搜索_签到日志.Size != 0 && Store.state.搜索_签到日志.Size != null) {
    对象_搜索条件.value = Store.state.搜索_签到日志
    console.log("恢复搜索条件")
    console.log(Store.state.搜索_签到日志.Size)
    console.log(Store.state.搜索_签到日志)
  }
...
onBeforeUnmount(() => {
  ...
  Store.commit("set搜索_签到日志", 对象_搜索条件.value)
})
